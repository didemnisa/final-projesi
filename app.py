...
def verifyAndRenderRespective():
	...
	try:
		if username == 'kasiyer' and password == 'kasiyer123':

			res = runQuery('call delete_old()')
			return render_template('cashier.html')

		elif username == 'müdür' and password == 'şifre123':
...
# Routes for cashier
@app.route('/Bu tarihte vizyondaki filmler', methods = ['POST'])
def moviesOnDate():
	...
	if res == []:
		return '<h4>Vizyonda film yoktur.</h4>'
	else:
...
def getPriceForClass():
	...
	if res == []:
		return '<h5>Bu filmin fiyatı henüz belirli değildir,lütfen tekrar deneyiniz!</h5>'

	price = int(res[0][0])
	if seatClass == 'gold':
		price = price * 1.5

	return '<h5>Bilet Fiyatı: ₺ '+str(price)+'</h5>\
...
def createBooking():
	...
	if res == []:
		return '<h5>Bilet başarıyla alındı!</h5>\
		<h6>Bilet Numarası: '+str(ticketNo)+'</h6>'
...
def insertMovie():
	...
	for i in res:
		if i[1] == movieName and i[2] == int(movieLen) and i[3] == movieLang \
		 and i[4].strftime('%Y/%m/%d') == startShowing and i[5].strftime('%Y/%m/%d') == endShowing:
			return '<h5>Aynı film zaten eklenmiş.</h5>'
	...
	if res == []:
		...
		if res == []:
			return '<h5>Film başarıyla eklendi!</h5>\
	...
	return '<h5>Bir şeyler ters gitti!</h5>'
...
def validMovies():
	...
	if res == []:
		return '<h5>Seçilen tarihte vizyonda olan film yok.</h5>'
...
def getHalls():
	...
	if availableHalls == set():
		...
		return '<h5> Vizyondaki filmler </h5>'
...
def priceList():
	...
	sortedDays = ['Pazartesi','Salı','Çarşamba','Perşembe','Cuma','Cumartesi','Pazar']
...
def runQuery(query):
	try:
		...
		if db.is_connected():
			print("MySQL ile bağlandı,Query çalıştırılıyor: ", query)
			...
			try:
				...
				print("Query hiçbir şey çalıştırmadı, ", e)
				return []
			return res
	...
	print("MySQL Database ile bağlantı sorunu yaşanıyor.")
